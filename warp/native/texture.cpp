...
void texture_add_descriptor(uint64_t id, const Texture2D& desc)
{
    g_texture_descriptors[id] = desc;
}

void texture_rem_descriptor(uint64_t id)
{
    g_texture_descriptors.erase(id);
}

} // namespace wp


WP_API uint64_t wp_texture_create_device(void* context,
                                  void* data_ptr,
                                  uint64_t size,
                                  int width,
                                  int height,
                                  ...
                                  int filter_mode)
{
    ...
    cudaArray_t cu_array = nullptr;
    {
        auto err = cudaMallocArray(
            &cu_array, &ch_desc, width, height, cudaArrayDefault);
    ...
    }
    {
        auto err = cudaMemcpy2DToArray(
            cu_array, 0, 0,
            data_ptr,
            (size_t)width * (size_t)channels * sizeof(unsigned char),
            (size_t)width * (size_t)channels * sizeof(unsigned char),
            (size_t)height,
            cudaMemcpyDefault);
        check_cuda(err);
    ...
    return id;
#else
    (void)context; (void)data_ptr; (void)size; (void)width; (void)height; (void)channels; (void)format; (void)normalized_coords; (void)address_mode; (void)filter_mode;
    return 0;
#endif
}
